...
export default function SummaryCards({ summary, unit }: SummaryCardsProps) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
      {cards.map((card, idx) => (
        <div key={idx} className={`glass-card-strong p-8 rounded-[40px] border ${card.borderColor} bg-gradient-to-br from-white/90 to-yellow-50/50 transition-all hover:scale-[1.05] hover:shadow-2xl hover:shadow-yellow-200/50 group`}>
          <div className="flex items-start justify-between mb-6">
            <div className={`p-4 rounded-[20px] ${card.bgColor} shadow-lg shadow-yellow-500/10 group-hover:scale-110 transition-transform`}>
